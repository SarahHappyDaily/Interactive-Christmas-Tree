...
export const foliageVertexShader = `
  ...
  void main() {
    ...
    // 3. Visibility Logic (Clean up the "crumbs")
    // When uProgress increases (scattering), we hide particles with aRandom < 0.8 (80% of them)
    float visibility = 1.0;
    if (aRandom < 0.8) {
        // Fade out quickly as we start scattering (between 0.0 and 0.3 progress)
        visibility = 1.0 - smoothstep(0.0, 0.3, uProgress);
    }
    ...
    // Apply visibility to size (effectively hiding them)
    gl_PointSize = (uSize * sizeVar * visibility) * (15.0 / -mvPosition.z);
